...
print(sort(significant_gene_names))
```
...
# PROBLEM: HOW SHOULD WE DECIDE THAT?
# Group by gene and take the mean length (or max or median or something)
gene_lengths_df <- gene_lengths_df %>%
  group_by(hgnc_symbol) %>%
  summarise(gene_length = round(mean(transcript_length))) 

# Convert to a named vector
gene_lengths <- setNames(gene_lengths_df$gene_length, gene_lengths_df$hgnc_symbol)
...
common_significant_genes <- intersect(significant_gene_names, rownames(tpm_counts))
...
for (gene in common_significant_genes) {
  # Extract the gene expression values from tpm_counts
  gene_values <- as.matrix(tpm_counts)[rownames(tpm_counts) == gene, ]
