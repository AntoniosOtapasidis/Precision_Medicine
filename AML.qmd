...
process_file <- function(file) {
  ...
  # Sum the stranded_first and stranded_second columns, and rename the sum
  df <- df %>%
    mutate(total_stranded = stranded_first + stranded_second) %>% 
    dplyr::select(-stranded_first, -stranded_second)

  # Rename the `total_stranded` column to the file name for further processing
  new_column_name <- gsub(".*/|\\.tsv$", "", file)
  df <- dplyr::rename(df, !!new_column_name := total_stranded)
