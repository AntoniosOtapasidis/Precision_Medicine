...
=======
  BiocManager::install("sva", update = TRUE, ask = FALSE)
>>>>>>> 473faedfec661c34704f5c8938be84f32999cb57
library(tidyverse)
library(readr)
library(biomaRt)
library(data.table)
...
results <- topTable(fit_limma, coef = "CR_statusYes", adjust.method = "BH", number = Inf)
...
volcano_plot <- ggplot(results, aes(x = logFC, y = -log10(P.Value), color = Significant)) +
...
nzv <- nearZeroVar(limma_corrected, saveMetrics = TRUE)

# Retain only columns with non-zero variance
limma_corrected <- limma_corrected[, !nzv$nzv]

# Check the dimensions of the filtered matrix
cat("Filtered matrix dimensions after removing near-zero variance columns:", dim(limma_corrected), "\n")

# Scale the filtered matrix explicitly
scaled_matrix <- scale(limma_corrected)
