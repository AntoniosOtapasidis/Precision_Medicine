...
expression_data <- merged_counts_with_ids[, -c(1:2)]

# Transpose the matrix to make samples rows and genes columns
expression_data <- t(as.matrix(expression_data))

# Assign row names from sample IDs
rownames(expression_data) <- colnames(merged_counts_with_ids)[-c(1:2)]
...
for (i in 1:ncol(expression_data)) {
  column <- expression_data[, i]
  column[is.na(column)] <- mean(column, na.rm = TRUE)
  expression_data[, i] <- column
}

# Standardize the data
standardized_data <- scale(expression_data)

# Remove non-numeric columns and transpose
expression_data <- merged_counts_with_ids[, -c(1:2)]
expression_data <- t(as.matrix(expression_data))
...
variances <- apply(expression_data, 2, var)
high_variance_features <- variances > 0.001  # Adjust threshold as needed
...
# Full model includes CR_status (the grouping variable)
...
## Based on the SVA we identified 2 surrogated variables
...
```{r}
plot(as.numeric(colData(dds)$Vital_Status), fit$sv[, 1], xlab = "Vital_Status", ylab = "SV1")
