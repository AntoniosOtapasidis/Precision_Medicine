...
ggplot(pca_data, aes(x = PC1, y = PC2, color = MRD_status, label = Sample)) +
  ...
  ggtitle("PCA Stratified by CR_status") +
...
It would be visual and meaning-wise better to perform PCA also based on the TPM counts (normalize for gene length and mainly for sequencing depth). We don't have the TPM values (we actually have them, but for the unstranded values).
